...
def open_data_entry(well_name):
    ...
    tk.Label(popup, text=f"Data for {well_name}", font=("Times New Roman", 12)).pack(
        pady=5
    )
...
def plot_well_history():
    for well, history in well_history.items():
        if not (
            any(v != 0 for v in history["od"]) or any(v != 0 for v in history["rfu"])
        ):
            continue
...
def select_and_plot_wells():
    ...
    tk.Label(frame, text="Select wells to plot").grid(
        row=0, column=0, columnspan=COLUMNS, pady=5
    )
    ...
    ).grid(row=1, column=0, columnspan=COLUMNS, pady=5)

    tk.Label(frame, text="Select by Promoter").grid(
        row=2, column=0, columnspan=COLUMNS, pady=5
    )
    ...
    for i, promoter in enumerate(promoter_groups):
        tk.Button(
            frame,
            text=f"Select Promoter: {promoter}",
    ...
    for r_i, r in enumerate(row_labels):
        for c in range(1, COLUMNS + 1):
            ...
            if history and (
                any(v != 0 for v in history["od"])
                or any(v != 0 for v in history["rfu"])
            ):
                var = tk.BooleanVar()
                well_vars[well] = var
                cb = tk.Checkbutton(
                    frame,
                    text=f"{well} ({history['promoter']} | {history['ahl']})",
                    variable=var,
                )
    ...
    def go_to_graph_type():
        ...
        if not selected_wells:
            messagebox.showwarning(
                "No wells selected", "Please select at least one well."
            )
            return
        graph_type_popup(selected_wells)

    tk.Button(frame, text="Next", command=go_to_graph_type).grid(
        row=ROWS + 3, column=0, columnspan=COLUMNS, pady=10
    )
    ...
    def graph_type_popup(selected_wells):
        ...
        #Radio button for Standard Graph
        tk.Radiobutton(popup, text="Standard Graph", variable=graph_type_var, value="all").pack(anchor="w", padx=10)
        
        #Radio button for Clustered Graph
        tk.Radiobutton(popup, text="Clustered Graph", variable=graph_type_var, value="clustered").pack(anchor="w", padx=10)

        #Radio button for RFU/OD Graph
        tk.Radiobutton(popup, text="RFU/OD - Time", variable=graph_type_var, value="RFUOD").pack(anchor="w", padx=10)

        show_legend_var = tk.BooleanVar(value=True)
        tk.Checkbutton(popup, text="Show Legend Window", variable=show_legend_var).pack(
            anchor="w", padx=10
        )
        ...
        def go_next():
            ...
            if graph_type_var.get() == "clustered":
                cluster_options_popup(selected_wells, show_legend_var.get())
            if graph_type_var.get() == "RFUOD":
                plot_rfu_od_time(selected_wells, show_legend_var.get())
    ...
    def cluster_options_popup(selected_wells, show_legend):
        ...
        for feat in ["total", "peak", "ending"]:
            ...
            tk.Checkbutton(popup, text=feat.capitalize(), variable=var).pack(
                anchor="w", padx=10
            )
        ...
        tk.Checkbutton(popup, text="Promoter", variable=include_promoter_var).pack(
            anchor="w", padx=10
        )
        tk.Checkbutton(popup, text="AHL Concentration", variable=include_ahl_var).pack(
            anchor="w", padx=10
        )
        ...
        tk.Radiobutton(
            popup, text="Automatic Clustering", variable=cluster_mode_var, value="auto"
        ).pack(anchor="w", padx=10)
        tk.Radiobutton(
            popup,
            text="Specify Number of Clusters",
            variable=cluster_mode_var,
            value="manual",
        ).pack(anchor="w", padx=10)
        ...
        def plot_clusters():
            ...
            if not any(signals_selected.values()):
                messagebox.showwarning(
                    "No measurement selected", "Select at least OD or RFU."
                )
                return
            ...
            if not features_selected:
                messagebox.showwarning(
                    ...
                    "Select at least one feature for clustering.",
                )
                return
            ...
            if cluster_mode_var.get() == "manual":
                try:
                    ...
                    messagebox.showwarning(
                        "Invalid input", "Enter a valid number of clusters."
                    )
                    return
    ...
    def plot_standard(selected_wells, show_legend=True):
        ...
        for well in selected_wells:
            ...
            ax1.plot(
                ...
                marker="o",
                linestyle="-",
                label=f"{history['promoter']} ({history['ahl']}) OD",
            ...
            ax2.plot(
                ...
                marker="x",
                linestyle="--",
                label=f"{history['promoter']} ({history['ahl']}) RFU",
                color=colors[color_index % len(colors)],
            )
            color_index += 1
        ...
        if show_legend:
            legend_fig = plt.figure(
                "Legend Window", figsize=(6, max(4, len(labels) * 0.35))
            )
    ...
    # --- Step 5: RFU/OD over time plotting function ---
    def plot_rfu_od_time(selected_wells, show_legend=True):
        ...
        for i, well in enumerate(selected_wells):
            ...
            valid_mask = od_vals != 0
            rounds_filtered = [r for r, m in zip(rounds, valid_mask) if m]
            if len(rounds_filtered) < 2:
                continue
            ratio = np.divide(
                rfu_vals[valid_mask],
                od_vals[valid_mask],
                out=np.full_like(rfu_vals[valid_mask], np.nan),
                where=od_vals[valid_mask] != 0,
            )
            ...
            line = ax.plot(
                ...
                label=f"{history['promoter']} ({history['ahl']}) {well}",
            )
            lines.append(line[0])
            labels.append(line[0].get_label())

        ax.set_xlabel("Round")
        ax.set_ylabel("RFU/OD")
        ax.set_title("RFU/OD - Time")
        fig.show()
...
def plot_clusters_gui(
    ...
    if signals_selected.get("OD", False):
        ...
        for cid, wells in od_clusters.items():
            ...
            mean_od = np.mean(
                [
                    np.pad(
                        v, (0, max_rounds - len(v)), "constant", constant_values=np.nan
                    )
                    for v in mean_od
                ],
                axis=0,
            ...
            line = ax_od.plot(
                ...
                linestyle="-",
                linewidth=2,
                label=f"Cluster {cid} OD (n={len(wells)})",
            )
    ...
    if signals_selected.get("RFU", False):
        ...
        rfu_cluster_ids = cp.cluster_signal(
            X_rfu, clustering_mode, n_clusters, dbscan_eps
        )
        ...
        for cid, wells in rfu_clusters.items():
            ...
            line = ax_rfu.plot(
                ...
                linestyle="--",
                linewidth=2,
                label=f"Cluster {cid} RFU (n={len(wells)})",
            )
    ...
    if show_legend:
        ...
        fig_legend, ax_legend = plt.subplots(
            figsize=(6, max(4, len(legend_items) * 0.35))
        )
        ax_legend.axis("off")
...
def build_controls():
    ...
    timer_label.grid(row=ROWS, column=0, columnspan=COLUMNS, pady=10)

    save_button = tk.Button(window, text="Save Current Data", command=save_plate_to_csv)
    save_button.grid(row=ROWS + 2, column=0, columnspan=COLUMNS, pady=5)

    round_button = tk.Button(window, text="Start New Round", command=start_round)
    round_button.grid(row=ROWS + 3, column=0, columnspan=COLUMNS, pady=5)

    plot_selected_button = tk.Button(
        window, text="Plot Selected Wells", command=select_and_plot_wells
    )
    plot_selected_button.grid(row=ROWS + 4, column=0, columnspan=COLUMNS, pady=10)
